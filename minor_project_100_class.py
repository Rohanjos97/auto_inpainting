...
class inpaint_defense:
        ...
        try:
            ...
            for folder in os.listdir():
                ...
                if folder.startswith('adv'):
                    for img, labels in images.take(100):
                        [self.process_image(image, lbl, 'adv', show_plot) for image, lbl in zip(img, labels)]

                if folder.startswith('clean'):
                    for img, labels in images.take(100):
                        [self.process_image(image, lbl, 'clean', show_plot) for image, lbl in zip(img, labels)]
    ...
    def process_image(self, image, label, data_type, plot=False):
        ...
        if data_type == 'adv':
            ...
            ind = int(class_list.index(images.class_names[label]) / 2)
...
for folder in os.listdir():
    ...
    if folder.startswith('adv'):
        for img, labels in images.take(100):
            [process_image(image, lbl) for image, lbl in zip(img, labels)]
reset_dir()
...
tmp = [x.append(img) for cls in lst for img in cls]
# np.array(tmp).shape
...
# Using tanh activation function
test = inpaint_defense(model, CLASS_INDEX, tf_act.tanh)
# id1.load_dataset('100class')
